...
DEBUG=0
TIME_PERIOD=3000
DEST_MACADDR="6c:b3:11:3d:53:a3"
SEND_PKTCNT=100000
MODE="Single"
...
main() {
    ...
    if [ "$2" = "icx" -o "$2" = "tgl" ]; then
        echo "Platform is $2"
    elif [ "$2" = "ehl" -o "$2" = "tglh" -o "$2" = "adl" -o "$2" = "tglh2" -o "$2" = "ehl2" -o "$2" = "adl2" ]; then
    ...
    if [ "$ACTION" = "setup" ]; then
	    ...
	    if [ "$MODE" = "single" ]; then
                ./talker/build/talker -l 2 -n 1 --vdev=net_af_xdp0,iface=$IFACE,start_queue=3 \
